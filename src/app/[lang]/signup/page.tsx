...
export default function SignupPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const handleSignUp = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // emailRedirectTo should be the confirmation page
        emailRedirectTo: `${location.origin}/auth/callback?next=/${lang}/parent/dashboard`,
      },
    });
    ...
    if (error) {
      ...
      // This could be a redirect to a dedicated "check your email" page
      alert(t('auth.signUpSuccessMessage'));
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="w-full max-w-md p-8 space-y-8 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('auth.createAccount')}</h1>
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">{t('auth.signUpMessage')}</p>
        </div>
        ...
        <p className="text-center text-sm text-gray-600 dark:text-gray-400">
          {t('auth.hasAccount')}{' '}
          <Link href={`/${lang}/login`} className="font-medium text-primary-600 hover:underline dark:text-primary-500">
            {t('auth.signInLink')}
          </Link>
        </p>
      </div>
